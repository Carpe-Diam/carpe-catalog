...
export default function GlobalHeader({ categoryTree, collections }: GlobalHeaderProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-[500] bg-background/80 backdrop-blur-md border-b border-white/5">
        <div className="px-6 md:px-12 py-6 flex items-center justify-between">
          <Link href="/" className="flex items-center hover:opacity-80 transition-opacity">
            <Image 
              ...
              width={140} 
              height={40} 
              className={`w-auto h-8 md:h-10 transition-opacity duration-500 brightness-0 invert ${showLogo ? 'opacity-100' : 'opacity-0'}`} 
              priority 
              unoptimized 
            />
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-10 text-[13px] tracking-[0.15em] font-medium text-foreground/70 uppercase">
            ...
            {hasCollections && (
              <div className="relative group">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 top-full pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                  <div className="bg-card border border-white/10 shadow-2xl min-w-[220px] py-3 rounded-xl overflow-hidden">
                    {displayCollections.map(col => (
                      <Link
                        key={col}
                        href={`/catalog?collection=${encodeURIComponent(col)}`}
                        className="block w-full text-left px-6 py-2.5 text-[12px] tracking-widest text-foreground/60 hover:text-foreground hover:bg-white/5 transition-all"
                      >
                        {col}
                      </Link>
                    ))}
            ...
            {hasCategories && (
              <div className="relative group">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 top-full pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                  <div className="bg-card border border-white/10 shadow-2xl min-w-[240px] py-3 rounded-xl overflow-hidden">
                    {categories.map(cat => (
                      <div key={cat} className="group/cat relative">
                        <Link
                          href={`/catalog?category=${encodeURIComponent(cat)}`}
                          className="flex items-center justify-between px-6 py-2.5 text-[12px] tracking-widest text-foreground/60 hover:text-foreground hover:bg-white/5 transition-all"
                        >
                        ...
                        {categoryTree[cat].length > 0 && (
                          <div className="absolute left-full top-0 ml-1 opacity-0 invisible group-hover/cat:opacity-100 group-hover/cat:visible transition-all duration-300">
                            <div className="bg-card border border-white/10 shadow-2xl min-w-[200px] py-3 rounded-xl overflow-hidden">
                              {categoryTree[cat].map(sub => (
                                <Link
                                  key={sub}
                                  href={`/catalog?category=${encodeURIComponent(cat)}&subcategory=${encodeURIComponent(sub)}`}
                                  className="block px-6 py-2 text-[12px] tracking-widest text-foreground/60 hover:text-foreground hover:bg-white/5 transition-all"
                                >
                                  {sub}
                                </Link>
                              ))}
