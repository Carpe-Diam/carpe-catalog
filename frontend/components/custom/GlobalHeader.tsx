...
export default function GlobalHeader({ categoryTree }: { categoryTree: Record<string, string[]> }) {
  ...
  const [showLogo, setShowLogo] = useState(!isHome); // visible immediately on non-home pages
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-4 flex items-center justify-between">
      <div className="flex items-center gap-12">
        ...
        <nav className="hidden lg:flex items-center gap-8 text-[11px] uppercase tracking-[0.2em] font-medium text-gray-500">
          ...
          {/* Fallback extra fixed links */}
          <Link href="/catalog?orderType=Customer%20order" className="hover:text-black transition-colors py-2">Bespoke</Link>
        </nav>
      </div>
      <div className="flex items-center gap-6 text-gray-400">
        <button className="hover:text-black transition-colors"><Search className="w-4 h-4" /></button>
        <button className="hover:text-black transition-colors"><Heart className="w-4 h-4" /></button>
        <button className="hover:text-black transition-colors"><ShoppingBag className="w-4 h-4" /></button>
      </div>
    </header>
  );
}
